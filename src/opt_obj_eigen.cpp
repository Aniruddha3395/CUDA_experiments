    ...
    double opt_obj::ErrFun(const std::vector<double> &x)
    {
        ...
        // auto gpu_start = std::chrono::high_resolution_clock::now();
            ...
            if (node_info(i,6)==1)
            {
                ...
                if (dot_prod > 1.0)
                    dot_prod = 1.0;
                shr_pot += tan( std::fabs(M_PI_2 - acos( dot_prod )) );
            }
            ...
            if (node_info(i,4)==1)
            {
                ...
                if (dot_prod > 1.0)
                    dot_prod = 1.0;
                angle = acos(dot_prod);
                bndg_pot += 1.124e10 * exp(-15.34 * angle) + 0.06109 * exp(-1.1386 * angle);
        ...
        // std::cout << "time : " << std::chrono::duration_cast<std::chrono::nanoseconds>(gpu_end - gpu_start).count() << " nanoseconds.\n";
        }

        // std::cout << properties(0,0) * grav_pot + properties(1,0) * spr_pot + properties(2,0) * shr_pot + properties(3,0) * bndg_pot << std::endl;
        return properties(0,0)*coords.col(2).array().abs().sum() + properties(1,0) * spr_pot + properties(2,0) * shr_pot + properties(3,0) * bndg_pot;
    };
