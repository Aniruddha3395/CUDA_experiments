...
namespace opt_obj
{
    ...
    double opt_obj::ErrFun_cuda(const std::vector<double> &x)
    {
        ...
        {
            coords(free_indices(i,0),0) = x[i];
            coords(free_indices(i,0),1) = x[i+1];
            coords(free_indices(i,0),2) = x[i+2];
        }
        ...
        return properties(0,0)*coords.col(2).array().abs().sum();
    }
    ...
    bool opt_obj::solveOPT()
    {
        ...
        // cudaMemcpy(ptr_node_info, node_info.data(), node_info.rows()*node_info.cols()*sizeof(double),cudaMemcpyHostToDevice);

        std::cout << "reached here" << std::endl;
        block_count = (int) no_particles/1000;
        ...
        {    
            ...
            successFlag = true;
            std::cout << "nlopt succeeded: " << std::endl;
        }
